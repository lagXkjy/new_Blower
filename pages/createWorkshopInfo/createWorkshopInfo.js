...
Page({
  ...
  getAddress() { //获取当前位置
    $api.getLocation()
      ...
      .then(res => {
        this.setData({ Address: res.result.address, address_component: res.result.address_component })
  ...
  submit(e) { //提交
    ...
    $api.loading('提交中...')
    // zfId //有id修改，无id添加
    $api.request(zfId ? $interface.UpdatezfInfo : $interface.CreateStation, 
      data, 
    )
      .then(res => {
        $api.hide()
